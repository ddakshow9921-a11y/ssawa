...
영수증/자필 메모 이미지 분석 기능을 사용하려면 Vercel 서버 환경변수에 `GEMINI_API_KEY`를 추가합니다. 이 값은 `VITE_` 접두사를 붙이지 않습니다.
`GEMINI_API_KEY`에는 Google AI Studio에서 만든 Gemini API 키를 넣습니다. 최신 Auth API Key는 `AQ.`로 시작할 수 있고 기존 Standard API Key는 `AIza`로 시작할 수 있습니다. Google OAuth 클라이언트 ID나 `GOCSPX-`로 시작하는 client secret은 Gemini 이미지 분석 호출에 사용할 수 없습니다.
사업자번호 기반 회원가입/입점 인증을 실제 국세청 API로 확인하려면 Vercel 서버 환경변수에 `NTS_BUSINESS_SERVICE_KEY`를 추가합니다. 이 값도 `VITE_` 접두사를 붙이지 않습니다.

## 배포 실패 가능 지점

- `VITE_SUPABASE_PUBLISHABLE_KEY` 미입력
- `GEMINI_API_KEY` 미입력 시 이미지 분석은 Gemini 대신 앱의 샘플 분석으로 fallback됨
- `NTS_BUSINESS_SERVICE_KEY` 미입력 시 사업자 인증은 수동 검토 접수로 fallback됨
