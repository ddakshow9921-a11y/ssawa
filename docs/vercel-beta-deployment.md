...
`GEMINI_API_KEY`에는 Google AI Studio에서 만든 `AIza`로 시작하는 Gemini API 키만 넣습니다. Google OAuth 클라이언트 ID, `GOCSPX-`로 시작하는 client secret, `AQ.`로 시작하는 액세스 토큰은 Gemini 이미지 분석 호출에 사용할 수 없습니다.
